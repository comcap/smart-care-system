...
async function renderDetail(id: string) {
  const store = createTestStore({ smartCare: { items: [sample], status: 'succeeded' } });
...
describe('RequestDetail screen', () => {
  it('renders the card with id, title, description, and createdAt when the request exists', async () => {
    ...
    expect(getByTestId('request-detail-card')).toBeTruthy();
    expect(getByText('sc-1')).toBeTruthy();
  ...
  it('shows an error state when the id does not match any request', async () => {
    ...
    expect(getByTestId('request-detail-error')).toBeTruthy();
    expect(queryByTestId('request-detail-card')).toBeNull();
  });
});
