...
describe('Main screen', () => {
  it('shows the empty state when there are no requests', async () => {
    ...
    expect(getByTestId('main-empty-state')).toBeTruthy();
    expect(queryByTestId('main-list')).toBeNull();
  });
  ...
  it('renders the list with one row per request when requests exist', async () => {
    ...
    expect(getByTestId('main-list')).toBeTruthy();
    expect(getByTestId('main-card-item-sc-1')).toBeTruthy();
    expect(queryByTestId('main-empty-state')).toBeNull();
  });

  it('navigates to RequestDetail when a card is pressed', async () => {
    const { getByTestId, navigation } = await renderMain([sample]);
    fireEvent.press(getByTestId('main-card-item-sc-1'));
    expect(navigation.navigate).toHaveBeenCalledWith('RequestDetail', { id: 'sc-1' });
  });

  it('navigates to AddRequest when the add button is pressed', async () => {
    const { getByTestId, navigation } = await renderMain([]);
    fireEvent.press(getByTestId('main-btn-add'));
  ...
  it('navigates to RequestDetail when searching an id that exists', async () => {
    const { getByTestId, navigation } = await renderMain([sample]);
    fireEvent.changeText(getByTestId('main-input-search'), 'sc-1');
    await flushMicrotasks();
    fireEvent.press(getByTestId('main-btn-search'));
    expect(navigation.navigate).toHaveBeenCalledWith('RequestDetail', { id: 'sc-1' });
  });

  it('shows the search-error modal instead of filtering the list when the id is not found', async () => {
    const { getByTestId, queryByTestId } = await renderMain([sample]);
    fireEvent.changeText(getByTestId('main-input-search'), 'does-not-exist');
    await flushMicrotasks();
    fireEvent.press(getByTestId('main-btn-search'));

    await waitFor(() => {
      expect(queryByTestId('main-modal-search-error', { includeHiddenElements: true })?.props.visible).toBe(
        true,
      );
    });
    expect(getByTestId('main-list', { includeHiddenElements: true })).toBeTruthy();
  });
  ...
  it('closes the search-error modal when confirm is pressed', async () => {
    ...
    await waitFor(() => {
      expect(getByTestId('main-modal-search-error', { includeHiddenElements: true }).props.visible).toBe(
        true,
      );
    });

    fireEvent.press(getByTestId('main-modal-btn-confirm'));

    await waitFor(() => {
      expect(queryByTestId('main-modal-search-error', { includeHiddenElements: true })).toBeNull();
    });
  });
});
