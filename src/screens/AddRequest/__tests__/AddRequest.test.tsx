...
describe('AddRequest screen', () => {
  it('renders the title and description inputs and submit button', async () => {
    ...
    expect(getByTestId('add-request__input--title')).toBeTruthy()
    expect(getByTestId('add-request__input--description')).toBeTruthy()
    expect(getByTestId('add-request__btn--submit')).toBeTruthy()
  })

  it('should disable submit until both title and description are filled', async () => {
    const { getByTestId } = await renderAddRequest()
    const submit = getByTestId('add-request__btn--submit')
    expect(submit.props.accessibilityState.disabled).toBe(true)

    fireEvent.changeText(getByTestId('add-request__input--title'), 'แอร์เสีย')
    await flushMicrotasks()
    fireEvent.changeText(
      getByTestId('add-request__input--description'),
      'แอร์ห้องประชุมไม่เย็น',
    )
